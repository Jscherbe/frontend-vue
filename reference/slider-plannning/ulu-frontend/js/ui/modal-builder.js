  ...
  if (config.autoIframe) {
    ...
    if (layout) {
      if (layout.isStaticSize) {
        modal.classList.add(`${ config.baseClass }--iframe-ratio`);
        body.style.aspectRatio = layout.aspectRatio;
      } else {
        modal.classList.add(`${ config.baseClass }--iframe-fill`);
        body.style.minHeight = layout.fillHeight || "65vh";
      }
    }
  }
