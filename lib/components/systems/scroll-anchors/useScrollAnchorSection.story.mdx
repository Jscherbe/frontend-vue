...
<template>
  <div :class="['my-custom-section', { 'is-active': isActive }]" :ref="element">
...
- `anchorId` (String, optional): A custom ID for the section's anchor. If not provided, one will be generated from the `title`.

## Returns

The composable returns an object with the following reactive properties:

- `element`: A ref that should be bound to the root HTML element of your section component. This element is observed by `UluScrollAnchors` to determine the section's visibility.
