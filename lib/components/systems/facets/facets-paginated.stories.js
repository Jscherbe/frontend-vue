...
const defaultSetup = (args) => {
  ...
  const { 
    ...
    isPinnedItem: args.isPinnedItem
  });
...
export const WithPinnedItems = (args) => ({
  components: {
    UluFacetsFilterAccordions,
    UluFacetsResults,
    UluFacetsSearch,
  ...
  template: `
    <UluFacetsSidebarLayout>
      ...
      <template #main>
        <UluFacetsResults :items="paginatedItems">
          <template #item="{ item }">
            <div style="border: 1px solid #eee; padding: 1rem; margin-bottom: 1rem; border-radius: 4px;" :style="args.isPinnedItem && args.isPinnedItem(item) ? 'border-color: #007bff; background-color: #f8faff;' : ''">
              <div style="display: flex; justify-content: space-between; align-items: flex-start;">
                <h3 style="margin-top: 0;">{{ item.title }}</h3>
                <span v-if="args.isPinnedItem && args.isPinnedItem(item)" style="background: #007bff; color: white; padding: 0.25rem 0.5rem; border-radius: 4px; font-size: 0.8rem; font-weight: bold;">Pinned</span>
              </div>
              <p>{{ item.description }}</p>
            </div>
          </template>
...
WithPinnedItems.args = {
  ...SidebarLayout.args,
  isPinnedItem: (item) => item.id === 3 || item.id === 8
};