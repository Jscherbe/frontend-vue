 ...
 * @param {Function} [options.isPinnedItem] - A function that receives an item and returns true if it should be pinned to the top of the results.
 */
export function useFacets(allItems, options = {}) {
  ...
  const displayItemsGrouped = computed(() => {
    ...
    let sorted = typeof sortFn === 'function' ? sortFn([...filteredItems.value]) : [...filteredItems.value];
    
    if (typeof isPinnedItem === 'function') {
      const pinned = [];
      const unpinned = [];
      sorted.forEach(item => isPinnedItem(item) ? pinned.push(item) : unpinned.push(item));
