import{g as o,d as s,o as r}from"./iframe-fc0isZdR.js";import"./preload-helper-BJwshlQW.js";const t={},p={style:{display:"none"}};function c(m,a){return r(),s("span",p)}const n=o(t,[["render",c]]);t.__docgenInfo={displayName:"UluEmpty",description:"",tags:{},sourceFiles:["/Users/joescherben/Personal/Projects/ULU/frontend-vue/lib/components/utils/UluEmpty.vue"]};const u={component:n,tags:["autodocs"],parameters:{docs:{description:{component:"A utility component that renders nothing visible. Useful as a placeholder or for conditional rendering where an empty element is needed."}}}},e={render:()=>({components:{UluEmpty:n},template:`
      <div>
        <p>This is some content before the UluEmpty component.</p>
        <UluEmpty />
        <p>This is some content after the UluEmpty component. You should not see any visible output from UluEmpty itself.</p>
      </div>
    `}),args:{}};e.parameters={...e.parameters,docs:{...e.parameters?.docs,source:{originalSource:`{
  render: () => ({
    components: {
      UluEmpty
    },
    template: \`
      <div>
        <p>This is some content before the UluEmpty component.</p>
        <UluEmpty />
        <p>This is some content after the UluEmpty component. You should not see any visible output from UluEmpty itself.</p>
      </div>
    \`
  }),
  args: {}
}`,...e.parameters?.docs?.source}}};const d=["Default"];export{e as Default,d as __namedExportsOrder,u as default};
